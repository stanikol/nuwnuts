...
object DAOUserDb {
  ...
  //    usr_storage.run(quote(query[UserDb].filter(_.email == lift(email)))).map(_.headOption)
  //  }
  //
  ...
  //        usr_storage.run(q).map(id => user.copy(id = id))
  //      case Some(usr) =>
  //        val q = quote(query[UserDb].filter(_.email == lift(usr.email)).update(lift(user)))
  //        usr_storage.run(q).map(_ => user.copy(id = usr.id))
  //    }
  //  }
  //
  ...
  //      case Some(_) =>
  //        usr_storage.run(quote(query[UserDb].filter(_.email == lift(usr.email)).delete))
